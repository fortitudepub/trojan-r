...
use crate::{
    ...
    protocol::{
        ...
        tls::{
            // f0p: disable tls accepptor used in server mod..
...
pub async fn launch_from_config_string(config_string: String) -> io::Result<()> {
    ...
    match config.mode.as_str() {
        #[cfg(feature = "server")]
        "server" => {
            // f0p: comment server mode beucase it relys on pki cert which
            // I have no time to find a way to do that using rust-native-tls
            log::debug!("server mode COMMETTED");
